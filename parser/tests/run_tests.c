...
const char *rec;
size_t total_size;
struct { char sid[6]; size_t sid_size; } obj;
...
START_TEST(parse_task_empty)
    struct kndTaskSpec specs[] = {{ .name = "sid", .name_size = strlen("sid"), .buf = obj.sid, .buf_size = &obj.sid_size, .max_buf_size = sizeof obj.sid }};

    rc = knd_parse_task(rec = "", &total_size, specs, sizeof specs / sizeof specs[0]);
    ck_assert(rc == knd_OK);
    ck_assert(total_size == strlen(rec));
    ck_assert(obj.sid_size == 0);
END_TEST
...
START_TEST(parse_value_max_size)
    ...
    ck_assert(rc == knd_OK);
    ck_assert(total_size == strlen(rec));
    ck_assert(obj.sid_size == strlen("123456") && !memcmp(obj.sid, "123456", obj.sid_size));
END_TEST
