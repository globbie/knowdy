...
static int
knd_parse_field_value(const char *name,
                      size_t name_size,
                      struct kndTaskSpec *spec,
                      const char *rec,
                      size_t *total_size,
                   ...
                   size_t num_specs)
{
    ...
    while (*c) {
        ...
        case ' ':
            ...
            err = knd_parse_field_value(b, e - b, spec, c, &chunk_size, &in_terminal);
            if (err) return err;
        ...
        case '}':
            ...
            err = knd_parse_field_value(b, e - b, spec, c, &chunk_size, &in_terminal);  // TODO(ki.stfu): allow in_terminal parsing
            if (err) return err;
                                  ...
                                  size_t num_specs)
{
    ...
    while (*c) {
        ...
        case ' ':
            ...
            if (DEBUG_PARSER_LEVEL_2)
                knd_log("++ got SPEC: \"%.*s\" default: %d  terminal: %d validate:%p",
                        spec->name_size, spec->name, spec->is_default,
                        spec->is_terminal, spec->validate);
                          ...
                          size_t num_specs)
{
    ...
    while (*c) {
		...
		if (!spec->is_list) {
		    knd_log("-- a list spec should handle the \"%.*s\" tag :(",
			    name_size, b);
		    return knd_FAIL;
		}
	    ...
	    if (!spec->is_list) {
                knd_log("-- not a list spec found to handle the \"%.*s\" list tag :(",
                        name_size, b);
                return knd_FAIL;
	    }
