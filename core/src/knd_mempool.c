                           ...
                           struct kndClassEntry **result)
{
    ...
    entry = &self->class_entries[self->num_class_entries];
    memset(entry, 0, sizeof(struct kndClassEntry));
    self->num_class_entries++;
    *result = entry;
    return knd_OK;
}
...
static int alloc(struct kndMemPool *self)
{
    ...
    self->class_entries = calloc(self->max_class_entries, sizeof(struct kndClassEntry));
...
parse_memory_settings(struct kndMemPool *self, const char *rec, size_t *total_size)
{
    struct gslTaskSpec specs[] = {
        ...
        {
            .name = "max_attr_vars",
            .name_size = strlen("max_attr_vars"),
            .parse = gsl_parse_size_t,
            .obj = &self->max_attr_vars
        },
        {
            .name = "max_attrs",
