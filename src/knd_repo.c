...
static int restore_state(struct kndRepo *self, struct kndTask *task)
{
    struct kndOutput *out = task->file_out;
...
int knd_repo_open(struct kndRepo *self, struct kndTask *task)
{
    ...
    /* read any existing commits from the WAL to the latest snapshot */
                          ...
                          struct kndTask *task)
{
    ...
    if (DEBUG_REPO_LEVEL_TMP)
        knd_log(".. commit #%zu to update the indices of %.*s [shard role:%d] class states:%p",
                commit->numid, self->name_size, self->name, task->role, commit->class_state_refs);
    ...
    for (ref = commit->class_state_refs; ref; ref = ref->next) {
        ...
        default:
            // KND_SELECTED
            knd_log("== children states:%p", ref->state->children);
