                                        ...
                                        const char *rec, size_t *total_size)
{
    ...
    err = validate_attr(self, name, name_size, &attr, &attr_inst, task);
...
static gsl_err_t import_attr_inst_list(void *obj,
                                       const char *name, size_t name_size,
                                       const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
                          ...
                          struct kndTask *task)
{
    ...
    if (!inst->name_size) {
        ...
        //knd_log("== autogenerated user name: %.*s", inst->entry->id_size, inst->entry->id);
        inst->entry->name = inst->entry->id;
        inst->entry->name_size = inst->entry->id_size;
