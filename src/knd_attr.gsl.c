                                 ...
                                 size_t depth)
{
    ...
    struct kndAttr *attr = parent_item->attr;
    struct kndClass *c = attr->parent_class;
    struct kndAttrVar *item;
    int err;

    if (DEBUG_ATTR_GSL_LEVEL_2) {
        knd_log(".. GSL export inner item: %.*s (class: %.*s)",
    ...
    if (parent_item->list) {
        err = attr_var_list_export_GSL(parent_item, task, depth + 1);
        if (err) return err;
        return knd_OK;
    }
                             ...
                             size_t depth)
{
    ...
    for (item = items; item; item = item->next) {
        ...
        err = out->write(out, "  {_of ", strlen("  {_of "));                      RET_ERR();
        err = out->write(out, attr->parent_class->name,
	...
	case KND_ATTR_ATTR_REF:
	    if (item->ref_attr) {
		err = knd_attr_export_GSL(item->ref_attr, task, depth + 1);       RET_ERR();
	    } else {
                                   ...
                                   size_t depth)
{
    ...
    err = out->write(out, attr->parent_class->name, attr->parent_class->name_size);  RET_ERR();
    err = out->write(out, " --}", strlen(" --}"));                      RET_ERR();
