...
static int query_plan(struct kndQuery *query, struct kndTask *task)
{
    ...
    FOREACH (stm, query->attr_stms) {
        err = knd_attr_stm_plan(stm, query->snapshot, task);
        switch (err) {
        case knd_OK:
...
gsl_err_t knd_query_process(void *obj, const char *rec, size_t *total_size)
{
    ...
    if (DEBUG_QUERY_RUN_LEVEL_TMP) {
        knd_log(">> task #%zu to process query", task->id);
    }
