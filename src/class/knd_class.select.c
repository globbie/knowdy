                                   ...
                                   const char *rec, size_t *total_size)
{
    ...
    err = knd_attr_find(bc, name, name_size, &attr, snapshot, task);
    if (err) {
...
static gsl_err_t import_class_inst(void *obj, const char *rec, size_t *total_size)
{
    ...
    err = knd_import_class_inst(c->entry, rec, total_size, snapshot, task);
...
int knd_cls_query_select(const char *rec, size_t *total_size, struct kndQuery *query, struct kndTask *task)
{
    ...
    if (DEBUG_CLASS_SELECT_LEVEL_2) {
        knd_log(".. parsing class select rec: \"%.*s\" {task-type %d}",
                32, rec, task->type);
    }
