#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include <time.h>

#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>
#include <sys/types.h>

/* numeric conversion by strtol */
#include <errno.h>
#include <limits.h>

#include "knd_config.h"
#include "knd_mempool.h"
#include "knd_state.h"
#include "knd_class.h"
#include "knd_class_inst.h"
#include "knd_attr.h"
#include "knd_attr_stm.h"
#include "knd_task.h"
#include "knd_user.h"
#include "knd_dict.h"
#include "knd_text.h"
#include "knd_rel.h"
#include "knd_quant.h"
#include "knd_proc.h"
#include "knd_proc_arg.h"
#include "knd_set.h"
#include "knd_shared_set.h"
#include "knd_logic.h"
#include "knd_utils.h"
#include "knd_ignore.h"
#include "knd_output.h"

#include <gsl-parser.h>

#define DEBUG_CLASS_IMPORT_LEVEL_1 0
#define DEBUG_CLASS_IMPORT_LEVEL_2 0
#define DEBUG_CLASS_IMPORT_LEVEL_3 0
#define DEBUG_CLASS_IMPORT_LEVEL_4 0
#define DEBUG_CLASS_IMPORT_LEVEL_5 0
#define DEBUG_CLASS_IMPORT_LEVEL_TMP 1

struct LocalContext {
    struct kndRepoSnapshot *snapshot;
    struct kndTask *task;
    struct kndClass *class;
    struct kndClassBasePred *base_pred;
};

static int register_cls_entry(struct kndClass *c, struct kndRepoSnapshot *snapshot, struct kndTask *task)
{
    struct kndCharSeq *seq;
    struct kndClassEntry *entry;
    int err;

    err = knd_get_cls_entry_by_name(snapshot, c->name, c->name_size, &entry, task);
    switch (err) {
    case knd_OK:
        return knd_CONFLICT;
    case knd_NO_MATCH:
        break;
    default:
        KND_TASK_ERR("failed to get {cls %.*s}", c->name_size, c->name);
        break;
    }

    err = knd_class_entry_new(&entry, task->mempool);
    KND_TASK_ERR("failed to alloc a cls entry");
    entry->name = c->name;
    entry->name_size = c->name_size;
    entry->cls = c;
    c->entry = entry;

    /* task local class name registration */
    err = knd_dict_set(task->idxs.cls_name_idx, c->name, c->name_size, (void*)entry, task);
    KND_TASK_ERR("failed to register a cls name");

    err = knd_charseq_register(snapshot, c->name, c->name_size, &seq, task);
    KND_TASK_ERR("failed to encode a cls name {seq %.*s}", c->name_size, c->name);
    entry->seq = seq;

    return knd_OK;
}

static gsl_err_t set_cls_name(void *obj, const char *name, size_t name_size)
{
    struct LocalContext *ctx = obj;
    struct kndClass *c = ctx->class;
    struct kndTask *task = ctx->task;
    struct kndRepoSnapshot *snapshot = ctx->snapshot;
    int err;

    c->name = name;
    c->name_size = name_size;
    knd_build_conc_abbr(name, name_size, c->abbr, &c->abbr_size);

    err = register_cls_entry(c, snapshot, task);
    if (err) return make_gsl_err_external(err);

    if (DEBUG_CLASS_IMPORT_LEVEL_3) {
        knd_log(">> registered {cls %.*s}", name_size, name);
    }
    return make_gsl_err(gsl_OK);
}

static gsl_err_t set_base_pred(void *obj, const char *name, size_t name_size)
{
    struct kndClassBasePred *bp = obj;
    if (!name_size) {
        knd_log("-- base class name not set");
        return make_gsl_err(gsl_FORMAT);
    }
    if (name_size >= KND_NAME_SIZE) return make_gsl_err(gsl_LIMIT);

    bp->name = name;
    bp->name_size = name_size;

    /* root cls */
    if (name_size == 1 && *name == '/') bp->is_root = true;

    return make_gsl_err(gsl_OK);
}

static gsl_err_t parse_logic_clause(void *obj, const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
    struct kndTask *task = ctx->task;
    struct kndLogicClause *clause;
    struct kndMemPool *mempool = task->mempool;
    int err;

    if (DEBUG_CLASS_IMPORT_LEVEL_2) {
        knd_log(".. parsing logic clause: \"%.*s\"", 32, rec);
    }
    err = knd_logic_clause_new(&clause, mempool);
    if (err) {
        KND_TASK_LOG("failed to alloc a logic clause");
        return *total_size = 0, make_gsl_err_external(knd_NOMEM);
    }

    err = knd_logic_clause_parse(clause, rec, total_size, task);
    if (err) return *total_size = 0, make_gsl_err_external(err);

    return make_gsl_err(gsl_OK);
}

static gsl_err_t parse_attr(void *obj, const char *name, size_t name_size,
                            const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
    struct kndClass *cls = ctx->class;
    struct kndTask *task = ctx->task;
    struct kndRepoSnapshot *snapshot = ctx->snapshot;
    struct kndAttr *attr;
    struct kndQuantAttr *quant_attr;
    struct kndClassRefAttr *cls_ref_attr;
    struct kndClassInnerAttr *cls_inner_attr;
    struct kndMemPool *mempool = task->mempool;
    struct kndText *tr = task->ctx->tr;
    size_t num_attr_types = sizeof(knd_attr_names) / sizeof(knd_attr_names[0]);
    const char *c;
    int err;

    task->ctx->tr = NULL;

    if (DEBUG_CLASS_IMPORT_LEVEL_2) {
        knd_log(".. parsing {attr %.*s} rec:\"%.*s\"", name_size, name, 32, rec);
    }
    err = knd_attr_new(&attr, mempool);
    if (err) {
        KND_TASK_LOG("failed to alloc an attr");
        return *total_size = 0, make_gsl_err_external(knd_NOMEM);
    }
    attr->owner = cls;

    for (size_t i = 0; i < num_attr_types; i++) {
        c = knd_attr_names[i];
        if (name_size != strlen(c)) continue;
        if (!memcmp(c, name, name_size)) {
            attr->type = (knd_attr_type)i;
            break;
        }
    }

    switch (attr->type) {
    case KND_ATTR_NONE:
        knd_log("{attr-type %.*s} is not supported for {cls %.*s}",
                name_size, name, cls->name_size, cls->name);
        return make_gsl_err_external(knd_NO_MATCH);
    case KND_ATTR_UINT:
        err = knd_quant_attr_new(&quant_attr, KND_QUANT_UINT, name, name_size, task->mempool);
        if (err) {
            KND_TASK_LOG("failed to alloc a quant attr");
            return *total_size = 0, make_gsl_err_external(knd_NOMEM);
        }
        attr->subtype = quant_attr;
        break;
    case KND_ATTR_UREAL:
        err = knd_quant_attr_new(&quant_attr, KND_QUANT_UREAL, name, name_size, task->mempool);
        if (err) {
            KND_TASK_LOG("failed to alloc a quant real attr");
            return *total_size = 0, make_gsl_err_external(knd_NOMEM);
        }
        attr->subtype = quant_attr;
        break;
    case KND_ATTR_CLS_REF:
        err = knd_cls_ref_attr_new(&cls_ref_attr, name, name_size, task->mempool);
        if (err) {
            KND_TASK_LOG("failed to alloc an attr cls ref");
            return *total_size = 0, make_gsl_err_external(knd_NOMEM);
        }
        attr->subtype = cls_ref_attr;
        break;
    case KND_ATTR_CLS_INNER:
        err = knd_cls_inner_attr_new(&cls_inner_attr, name, name_size, task->mempool);
        if (err) {
            KND_TASK_LOG("failed to alloc an attr cls inner");
            return *total_size = 0, make_gsl_err_external(knd_NOMEM);
        }
        attr->subtype = cls_inner_attr;
        break;
    default:
        break;
    }

    err = knd_attr_import(attr, rec, total_size, snapshot, task);
    if (err) {
        KND_TASK_LOG("failed to import an attr");
        return *total_size = 0, make_gsl_err_external(err);
    }

    knd_class_append_attr(cls, attr);

    /* restore owner's glosses */
    task->ctx->tr = tr;
    return make_gsl_err(gsl_OK);
}

static gsl_err_t import_attr_stm(void *obj, const char *name, size_t name_size,
                                 const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
    struct kndTask *task = ctx->task;
    struct kndClassBasePred *bp = ctx->base_pred;
    struct kndAttrStm *stm;
    int err;

    err = knd_attr_stm_new(&stm, bp->subj, task->mempool);
    if (err) {
        KND_TASK_LOG("failed to alloc an attr stm");
        return *total_size = 0, make_gsl_err_external(knd_NOMEM);
    }
    stm->name = name;
    stm->name_size = name_size;

    err = knd_import_attr_stm(stm, name, name_size, rec, total_size, ctx->task);
    if (err) return *total_size = 0, make_gsl_err_external(err);

    knd_base_pred_append_attr_stm(bp, stm);

    return make_gsl_err(gsl_OK);
}

static gsl_err_t import_attr_stm_list(void *obj, const char *name, size_t name_size,
                                      const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
    struct kndTask *task = ctx->task;
    struct kndClassBasePred *bp = ctx->base_pred;
    struct kndAttrStm *stm;
    int err;

    if (DEBUG_CLASS_IMPORT_LEVEL_2) {
        knd_log(">> import attr stm list [%.*s ..]", name_size, name);
    }

    err = knd_attr_stm_new(&stm, bp->subj, task->mempool);
    if (err) {
        KND_TASK_LOG("failed to alloc an attr stm");
        return *total_size = 0, make_gsl_err_external(knd_NOMEM);
    }
    stm->name = name;
    stm->name_size = name_size;

    err = knd_import_attr_stm_list(stm, name, name_size, rec, total_size, ctx->task);
    if (err) return *total_size = 0, make_gsl_err_external(err);

    assert (stm->list != NULL);

    knd_base_pred_append_attr_stm(bp, stm);

    return make_gsl_err(gsl_OK);
}

static gsl_err_t parse_base_pred(const char *rec, size_t *total_size, struct LocalContext *ctx)
{
    gsl_err_t parser_err;

    struct gslTaskSpec specs[] = {
        { .is_implied = true,
          .run = set_base_pred,
          .obj = ctx->base_pred
        },
        { .validate = import_attr_stm,
          .obj = ctx
        },
        { .type = GSL_GET_ARRAY_STATE,
          .validate = import_attr_stm_list,
          .obj = ctx
        },
        { .name = "_pred",
          .name_size = strlen("_pred"),
          .parse = parse_logic_clause,
          .obj = ctx
        }
    };

    parser_err = gsl_parse_task(rec, total_size, specs, sizeof specs / sizeof specs[0]);
    if (parser_err.code) return parser_err;

    return make_gsl_err(gsl_OK);
}

static gsl_err_t parse_baseclass(void *obj, const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
    struct kndTask *task = ctx->task;
    struct kndClass *cls = ctx->class;
    struct kndClassBasePred *base_pred;
    struct kndMemPool *mempool = task->mempool;
    gsl_err_t parser_err;
    int err;

    if (DEBUG_CLASS_IMPORT_LEVEL_2) {
        knd_log(".. parsing the base {cls %.*s}", 32, rec);
    }

    err = knd_class_base_pred_new(&base_pred, cls, mempool);
    if (err) {
        KND_TASK_LOG("failed to alloc a base pred");
        return *total_size = 0, make_gsl_err_external(knd_NOMEM);
    }
    ctx->base_pred = base_pred;

    parser_err = parse_base_pred(rec, total_size, ctx);
    if (parser_err.code) return parser_err;

    knd_class_append_base_pred(cls, base_pred);

    return make_gsl_err(gsl_OK);
}

static gsl_err_t set_uniq_type(void *obj, const char *name, size_t name_size)
{
    struct LocalContext *ctx = obj;
    struct kndClass *cls = ctx->class;

    if (DEBUG_CLASS_IMPORT_LEVEL_2)
        knd_log(".. set uniq type: \"%.*s\" for \"%.*s\"",
                name_size, name, cls->name_size, cls->name);

    return make_gsl_err(gsl_OK);
}

static gsl_err_t add_uniq_attr(void *obj, const char *name, size_t name_size,
                               const char *unused_var(rec), size_t *total_size)
{
    struct LocalContext *ctx = obj;
    struct kndClass *cls = ctx->class;
    struct kndTask *task = ctx->task;
    struct kndMemPool *mempool = task->mempool;
    struct kndAttrRef *ref;
    int err;

    if (DEBUG_CLASS_IMPORT_LEVEL_2)
        knd_log(">> add uniq attr: \"%.*s\"", name_size, name);

    err = knd_attr_ref_new(&ref, mempool);
    if (err) {
        KND_TASK_LOG("failed to alloc kndAttrRef");
        return *total_size = 0, make_gsl_err_external(knd_NOMEM);
    }
    ref->name = name;
    ref->name_size = name_size;

    ref->next = cls->uniq;
    cls->uniq = ref;
    return make_gsl_err(gsl_OK);
}

static gsl_err_t parse_uniq_attr_constraint(void *obj, const char *rec, size_t *total_size)
{
    struct LocalContext *ctx = obj;
    gsl_err_t parser_err;

    struct gslTaskSpec specs[] = {
        { .is_implied = true,
          .run = set_uniq_type,
          .obj = ctx
        },
        { .validate = add_uniq_attr,
          .obj = ctx
        }
    };

    parser_err = gsl_parse_task(rec, total_size, specs, sizeof specs / sizeof specs[0]);
    if (parser_err.code) return parser_err;

    return make_gsl_err(gsl_OK);
}

int knd_class_import(const char *rec, size_t *total_size, struct kndClass **result,
                     struct kndRepoSnapshot *snapshot, struct kndTask *task)
{
    struct kndClass *cls;
    gsl_err_t parser_err;
    int err;

    if (DEBUG_CLASS_IMPORT_LEVEL_2) {
        knd_log(">> import {cls %.*s}", 128, rec);
    }

    err = knd_class_new(&cls, task->mempool);
    KND_TASK_ERR("failed to alloc a class");

    struct LocalContext ctx = {
        .snapshot = snapshot,
        .task = task,
        .class = cls
    };

    struct gslTaskSpec specs[] = {
        { .is_implied = true,
          .run = set_cls_name,
          .obj = &ctx
        },
        { .name = "is",
          .name_size = strlen("is"),
          .parse = parse_baseclass,
          .obj = &ctx
        },
        { .type = GSL_GET_ARRAY_STATE,
          .name = "gloss",
          .name_size = strlen("gloss"),
          .parse = knd_parse_gloss_array,
          .obj = &ctx
        },
        { .name = "uniq",
          .name_size = strlen("uniq"),
          .parse = parse_uniq_attr_constraint,
          .obj = &ctx
        },
        { .name = "clause",
          .name_size = strlen("clause"),
          .parse = knd_ignore_obj,
          .obj = &ctx
        },
        { .validate = parse_attr,
          .obj = &ctx
        }
    };

    parser_err = gsl_parse_task(rec, total_size, specs, sizeof specs / sizeof specs[0]);
    if (parser_err.code) {
        err = gsl_err_to_knd_err_codes(parser_err);
        switch (err) {
        case knd_NO_MATCH:
            KND_TASK_ERR("unrecognized {tag %.*s} in {cls %.*s}",
                         parser_err.val_size, parser_err.val, cls->name_size, cls->name);
            break;
        default:
            KND_TASK_ERR("{cls %.*s} parsing failed {err %d}", cls->name_size, cls->name, err);
            break;
        }
    }

    if (!cls->name_size) {
        err = knd_FORMAT;
        KND_TASK_ERR("no cls name specified");
    }

    /* reassign glosses */
    if (task->ctx->tr) {
        cls->entry->glosses = task->ctx->tr;
        task->ctx->tr = NULL;
    }

    cls->phase = KND_CLASS_IMPORTED;

    if (DEBUG_CLASS_IMPORT_LEVEL_3) {
        knd_log("++  {cls %.*s} import completed!", cls->name_size, cls->name);
    }

#if 0
    switch (task->type) {
    case KND_TASK_RESTORE:
        // fall through
    case KND_TASK_COMMIT:
        err = knd_class_commit_state(cl->entry, KND_CREATED, task);
        KND_TASK_ERR("failed to commit a task state");
        break;
    default:
        break;
    }
#endif

    *result = cls;
    return knd_OK;
}
