...
static int match_elem(void *elem, void *ctx_obj)
{
    ...
    if (key) {
        err = spec->key_get_cb(elem, (void*)snapshot, &elem_key, task);
        KND_TASK_ERR("failed to obtain a facet key from elem");

        if (elem_key != key) {
            err = spec->hash_cb(key, NULL, elem_key, (void*)snapshot, &next_key, &numval, task);
            switch (err) {
            case knd_OK:
                break;
            case knd_NO_MATCH:
...
static int apply_cb(struct kndFacet *facet, void *key, struct kndSetRange *unused_var(range),
                    ...
                    struct kndRepoSnapshot *snapshot, struct kndTask *task)
{
    ...
    for (size_t i = 0; i < KND_MAX_FACETS; i++) {
        ...
        err = apply_cb(facet->children[i], key, NULL,
                       filter_cb, filter_ctx, map_cb, map_ctx, snapshot, task);
...
/**
 *   find a matching facet and apply map_cb to each of its elems
                  ...
                  struct kndRepoSnapshot *snapshot, struct kndTask *task)
{
    ...
    if (!key) {
        err = apply_cb(facet, NULL, range, filter_cb, filter_ctx, map_cb, map_ctx, snapshot, task);
    ...
    if (!facet->num_children) {
        err = apply_cb(facet, key, range, filter_cb, filter_ctx, map_cb, map_ctx, snapshot, task);
        KND_TASK_ERR("failed to apply a cb to a terminal facet");
        return knd_OK;
    }

    err = spec->hash_cb(facet->key, NULL, key, (void*)snapshot, &next_key, &numval, task);
    KND_TASK_ERR("failed to apply a facet hash func {err %d}", err);
    ...
    err = knd_facet_map(f, key, range, filter_cb, filter_ctx, map_cb, map_ctx, snapshot, task);
